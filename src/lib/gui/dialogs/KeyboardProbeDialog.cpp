...
{
  setWindowTitle(QStringLiteral("Keyboard Probe"));
  setWindowFlags(windowFlags() | Qt::WindowStaysOnTopHint);
  setFixedSize(720, 420);
  setFocusPolicy(Qt::StrongFocus);

  auto *layout = new QVBoxLayout(this);
  m_statusLabel = new QLabel(QStringLiteral("Focus this window and press/release keys."), this);
...
void KeyboardProbeDialog::appendKeyEvent(const QString &eventType, const QKeyEvent *event)
{
  ...
  const auto line =
      QStringLiteral("[%1] %2 key=0x%3(%4) text=%5 scan=0x%6 nativeVk=0x%7 modifiers=%8 nativeModifiers=0x%9")
          .arg(
  ...
  appendLine(line);
  m_statusLabel->setText(QStringLiteral("Captured %1 in %2").arg(eventType, event->text()));
}
...
void KeyboardProbeDialog::keyPressEvent(QKeyEvent *event)
{
  ...
  appendKeyEvent(QStringLiteral("DOWN"), event);
  event->accept();
}
...
void KeyboardProbeDialog::keyReleaseEvent(QKeyEvent *event)
{
  ...
  appendKeyEvent(QStringLiteral("UP"), event);
  event->accept();
}
